...
2.
z0.y = sin(phase)*c.x;
z0.x = sin(phase*5)*c.y;

3.
z0.y = sin(phase) + sin(c.x*10);
z0.x = cos(phase) + sin(c.x*10);

4.
vec4 e1 = vec4(cos(phase*3), sin(phase*3), 0, 0);
vec4 e2 = vec4(-sin(phase*3), cos(phase*3), 0, 0);
vec4 off = vec4(0, 0, sin(cs[0].x*10), 0);
c = e1.xy * cs[0].x + e2.xy * cs[0].y + off.xy;
z0 = e1.zw * cs[0].x + e2.zw * cs[0].y + off.zw;

5.
vec4 e1 = vec4(cos(phase*3), sin(phase*3), 0, 0);
vec4 e2 = vec4(-sin(phase*3), cos(phase*3), 0, 0);
vec4 off = vec4(0, 0, sin(cs[0].x*3)/4, 0);
c = e1.xy * cs[0].x + e2.xy * cs[0].y + off.xy;
z0 = e1.zw * cs[0].x + e2.zw * cs[0].y + off.zw;
...
render_fractal = () ->
    x = 0 # (Math.random() - 0.5) * cell_size_x
    y = 0 # (Math.random() - 0.5) * cell_size_y
    rotation = 0 # Math.random() * Math.PI * 2
    phase = (new Date().getTime() - t0) / 1000
...
render = () ->
    ...
    GL.uniform1f(GL.getUniformLocation(program_composite, "max_counter"), 20)
    if sz[0] < sz[1]
        y_scale = sz[0] / sz[1]
        x_scale = 1
    else
