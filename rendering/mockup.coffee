...
zmq_sub.on("message", (buffer) ->
    ...
    parameters['x_diff'] = Math.abs(state.left.position[0] - state.right.position[0]) / 200

    v1x = state.left.normal[0]
    v1y = state.left.normal[1]
    v1l = Math.sqrt(v1x * v1x + v1y * v1y)
    v1x /= v1l
    v1y /= v1l

    v2x = state.right.normal[0]
    v2y = state.right.normal[1]
    v2l = Math.sqrt(v2x * v2x + v2y * v2y)
    v2x /= v2l
    v2y /= v2l

    angle = Math.asin(v1x * v2y - v1y * v2x)
...
geometry_shader = """
    ...
    void main() {
        ...
        float theta = angle;
        // vec4 e1 = vec4(cos(theta), 0, 0, sin(theta)) / 2;
        // vec4 e2 = vec4(0, cos(theta), sin(theta), 0) / 2;

        vec4 e1 = vec4(cos(theta), 0, sin(theta), 0) / 2 * scale;
        vec4 e2 = vec4(0, cos(theta), 0, sin(theta)) / 2 * scale;
...
setupBuffers = () ->
    ...
    @colormap_image = graphics.loadImageData(require("fs").readFileSync(__dirname + "/gradient2.png"));
...
render = () ->
    ...
    GL.uniform1f(GL.getUniformLocation(program_composite, "max_counter"), 500 * pixel_size * vertices / 1000000)
    if sz[0] < sz[1]
        y_scale = sz[0] / sz[1]
        x_scale = 1
    else
